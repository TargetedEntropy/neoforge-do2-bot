    ...
    for (player_entity, packet) in events_owned {
        match packet {
            ...
            ClientboundConfigurationPacket::CustomPayload(p) => {
                ...
                let id_str = p.identifier.to_string();
                if id_str == "c:version" {
                    debug!("Responding to NeoForge c:version negotiation");
                    let mut system_state: SystemState<Query<&RawConnection>> =
                        SystemState::new(ecs);
                    let mut query = system_state.get_mut(ecs);
                    let raw_connection = query.get_mut(player_entity).unwrap();
                    // Respond with version list [1]: VarInt list_length=1, VarInt version=1
                    let response_data: Vec<u8> = vec![0x01, 0x01];
                    raw_connection
                        .write_packet(
                            azalea_protocol::packets::configuration::serverbound_custom_payload_packet::ServerboundCustomPayloadPacket {
                                identifier: azalea_core::resource_location::ResourceLocation::new("c:version"),
                                data: response_data.into(),
                            }
                            .get(),
                        )
                        .unwrap();
                } else if id_str == "c:register" {
                    debug!("Responding to NeoForge c:register negotiation");
                    let mut system_state: SystemState<Query<&RawConnection>> =
                        SystemState::new(ecs);
                    let mut query = system_state.get_mut(ecs);
                    let raw_connection = query.get_mut(player_entity).unwrap();
                    // Respond: version=1, protocol="play" (STRING_UTF8), channels=0
                    // NeoForge CommonRegisterPayload uses STRING_UTF8 for the protocol field
                    let response_data: Vec<u8> = vec![
                        0x01,                          // version = 1 (VarInt)
                        0x04, 0x70, 0x6C, 0x61, 0x79,  // STRING_UTF8("play")
                        0x00,                          // channel count = 0
                    ];
                    raw_connection
                        .write_packet(
                            azalea_protocol::packets::configuration::serverbound_custom_payload_packet::ServerboundCustomPayloadPacket {
                                identifier: azalea_core::resource_location::ResourceLocation::new("c:register"),
                                data: response_data.into(),
                            }
                            .get(),
                        )
