            ...
            match packet {
                ...
                ClientboundLoginPacket::GameProfile(p) => {
                    ...
                    conn.write(ServerboundLoginAcknowledgedPacket {}.get())
                        .await?;
                    let mut config_conn = conn.configuration();

                    // Register c:version and c:register channels so the server
                    // knows we can handle the NeoForge common networking handshake.
                    // The server-side negotiation is patched to always succeed,
                    // so we don't need to send neoforge:register.
                    let register_data: Vec<u8> = b"c:version\0c:register".to_vec();
                    config_conn.write(
                        azalea_protocol::packets::configuration::serverbound_custom_payload_packet::ServerboundCustomPayloadPacket {
                            identifier: azalea_core::resource_location::ResourceLocation::new("register"),
                            data: register_data.into(),
                        }.get()
                    ).await?;
                    debug!("Sent channel registration (c:version, c:register)");

                    break (config_conn, p.game_profile);
                }
